	...
	function makeView(tps,out,twds,wds,curc,caut,cit) {
		...
		if(cit==-1) {
			if(caut!=-1) out.push("<h1 style=\"font-size:2.4em\"><a "+OC("-a")+" class=\"clc\">×</a> <b>"+tps.authors[caut][1]+"</b></h1>");
	...
	function makeItem(tps, ind,cts,lst, big) {
		...
		var cat = cts["c"+tpl[5]];
		lst.push("<span>")
		lst.push( "By <a class=\"clc\" "+OC("a-"+au[0]     )+">" +(big?au[1]:shorten(au[1],~~(lim*0.7)))+"</a>");
